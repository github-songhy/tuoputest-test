...
function updateTopologyStatus() {
    ...
    fetch('http://localhost:3000/api/devices')
        ...
        .then(newDevices => {
            ...
            // 新添加的逻辑：传播流动效果到整个路径
            // 1. 首先为所有连线创建映射，便于快速查找

            const connectionMap = new Map();
            connections.forEach(conn => connectionMap.set(conn.id, conn));
            console.log(connections)

            // 2. 遍历所有设备 找到每个设备的正在活跃(有流动效果)的最源头，如果有，就为以这个设备为起点的连线添加流动效果
            usedDevices.forEach(device => {
                const deviceId = device.id;
                ...
                //     // 5. 查找从源头设备到当前设备的路径上的所有连线
                //     const pathConnections = findPathConnections(sourceId, deviceId);

                //     // 6. 检查路径的起始连线是否有流动效果
                //     let hasStartingFlow = false;
                //     pathConnections.forEach(connId => {
                //         const conn = connectionMap.get(connId);
                //         if (conn && conn.source === sourceId && conn.flowGroup) {
                //             hasStartingFlow = true;
                //         }
                //     });

                //     // 7. 如果起始连线有流动效果，则为整个路径添加流动效果
                //     if (hasStartingFlow) {
                //         pathConnections.forEach(connId => {
                //             const conn = connectionMap.get(connId);
                //             if (conn && !conn.flowGroup) { 
                //                 // conn.flowGroup如果有值,取反就是false,反之是true
                //                 addFlowEffect(connId);
                //             }
                //         });
                //     }
                // });
